...
public class SearchBar extends GraphicComponent {
	...
	//draw -----------------------------------------
	public void draw() {
		rShapes.DrawRectangleRec(border, Color.GOLD);
		Finestra.getRaylib().text.DrawText(getDisplayedText(), (int)(border.x), (int)(border.y), fontSize, /*foregroundColor*/ Color.BEIGE);
//		Finestra.getRaylib().text.DrawText(String.valueOf(nome), contaFrame, back, MAX_INPUT_CHARS, null);
	}
	...
	private String getDisplayedText() {
		...
		return String.valueOf(nome).substring(Math.max(0, nLettere-maxCharDisplayed), nLettere);
	}
